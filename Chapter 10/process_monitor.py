...
def get_process_privileges(pid):
	try:
		...
		for i in privs:
			...
			if i[1] == 3:
				priv_list += "%s|" % win32security.LookipPrivilegeName(None,i[0])
...
def log_to_file(message):
	fd = open("process_monitor_log.cvs","ab")
	fd.write("%s\r\n" % message)
	fd.close()
...
while True:
	try:
		...
		process_log_message = "%s,%s,%s,%s,%s,%s,\r\n" % (create_date,proc_owner,executable,cmdline,pid,parent pid,privileges)
